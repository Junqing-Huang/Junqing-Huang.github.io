...
`);
  ...
  // const columnWidth = (1024 - 2 * marginLarge - numGutters * gutter) / numCols;
  //
  ...
  ***/
  ...
  <div class="byline">
    ...
    <div style="text-transform:uppercase;">
      ${frontMatter.authors.map(author => `
          ${author.personalURL ? `
            <b><a class="name" href="${author.personalURL}">${author.name}</a></b>` : `
            <span class="name">${author.name}</span>`}
      `).join(', ')}
      </div>
    </div>
  ...
  <div class="byline">
    <div class="authors-affiliations grid">
      ${frontMatter.publishedDate ? `
        <h3>Published:    ${frontMatter.publishedMonth} ${frontMatter.publishedDay}, ${frontMatter.publishedYear}</h3> ` : `
        <p><em>Not published yet.</em></p>`}
    </div>
  </div>
`;
